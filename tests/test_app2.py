...
from apps.app2.app2_main import app
...
TEST_OPERATIONS = [
    (LedgerOperation.DAILY_REWARD, APP2_LEDGER_CONFIG["DAILY_REWARD"]),
    (LedgerOperation.SIGNUP_CREDIT, APP2_LEDGER_CONFIG["SIGNUP_CREDIT"]),
]
...
def test_create_valid_entry(client, ledger_service, db_session):
    payload = {
        ...
        "amount": APP2_LEDGER_CONFIG["DAILY_REWARD"],
        "nonce": TEST_NONCE,
        "owner_id": TEST_USER,
...
def test_duplicate_nonce_rejection(client):
    payload = {
        ...
        "amount": APP2_LEDGER_CONFIG["SIGNUP_CREDIT"],
        "nonce": TEST_NONCE,
        "owner_id": TEST_USER,
    }

    response1 = client.post("/app2/ledger", json=payload)
    assert response1.status_code == 200

    # this one intentionally fails
    response2 = client.post("/app2/ledger", json=payload)
...
def test_insufficient_balance(client):
    ...
    payload = {
        ...
        "amount": APP2_LEDGER_CONFIG["CREDIT_SPEND"],
        "nonce": TEST_NONCE,
        "owner_id": TEST_USER,
    }
...
def test_mixed_operations(client, ledger_service):
    ...
    credits = [(LedgerOperation.DAILY_REWARD, 1), (LedgerOperation.CREDIT_ADD, 10)]

    debits = [(LedgerOperation.CREDIT_SPEND, -1)]
